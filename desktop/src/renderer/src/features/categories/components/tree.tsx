...
export function CategoryTree({ category, openAddCategoryModal, depth = 0 }: CategoryTreeProps) {
  ...
  if (!category.subCategories || category.subCategories.length === 0) {
    return (
      <div className="flex justify-between items-center gap-x-3">
        <div className="grid grid-cols-6 items-center w-full py-2 pr-3 rounded-md">
          <div
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center gap-x-3">
        <CollapsibleTrigger className="grid grid-cols-6 items-center w-full py-2 pr-3 hover:bg-muted/50 rounded-md">
          <div style={{ paddingLeft }} className="flex col-span-4 items-center gap-x-2">
            <ChevronRight
